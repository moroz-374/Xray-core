...
| Shadowsocks 2022 single | TCP and UDP connection metadata from sing-box adapter | Accepted message has configured single-user email | Direct global `Dispatch` for TCP/UDP in the pinned implementation | Both networks, domain/IP variants, metadata conversion and payload sniffing |
