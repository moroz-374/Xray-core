...
func TestVlessWebSocketTLSMatrix(t *testing.T) {
	testVlessTransportTLS(t, "websocket", []*internet.TransportConfig{{
		ProtocolName: "websocket",
		Settings:     serial.ToTypedMessage(&websocket.Config{Path: "/x40-vless"}),
	}})
}

func TestVlessGRPCTLSMatrix(t *testing.T) {
	testVlessTransportTLS(t, "grpc", []*internet.TransportConfig{{
		ProtocolName: "grpc",
		Settings:     serial.ToTypedMessage(&grpc.Config{ServiceName: "x40vless"}),
	}})
}

func TestVlessHTTPUpgradeTLSMatrix(t *testing.T) {
	testVlessTransportTLS(t, "httpupgrade", []*internet.TransportConfig{{
		ProtocolName: "httpupgrade",
		Settings:     serial.ToTypedMessage(&httpupgrade.Config{Path: "/x40-vless"}),
	}})
}

func testVlessTransportTLS(t *testing.T, transportProtocol string, transportSettings []*internet.TransportConfig) {
	t.Helper()
	...
	serverConfig := &core.Config{
		Inbound: []*core.InboundHandlerConfig{{
			...
			ProxySettings: serial.ToTypedMessage(&vin.Config{Clients: []*protocol.User{{
	...
	clientConfig := &core.Config{
		...
		Outbound: []*core.OutboundHandlerConfig{{
			ProxySettings: serial.ToTypedMessage(&vout.Config{Vnext: &protocol.ServerEndpoint{
